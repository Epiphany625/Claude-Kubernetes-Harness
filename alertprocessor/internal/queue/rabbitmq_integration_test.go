...
// The dead-letter pair has to exist before a consumer rejects anything, or the
// rejected message is dropped rather than parked.
func TestDeadLetterTopologyExists(t *testing.T) {
	ctx := context.Background()
	url := startBroker(t)
	...
	// Passive declare: succeeds only if the object already exists with the same
	// arguments, and closes the channel otherwise.
	if _, err := ch.QueueDeclarePassive("agent.events.dlq", true, false, false, false,
		amqp.Table{"x-queue-type": "quorum"}); err != nil {
		t.Errorf("dead-letter queue agent.events.dlq was not declared: %v", err)
	}
}

// A rejected message must reach the DLQ rather than vanish -- this is what makes
// a poison event debuggable instead of a mystery.
func TestRejectedMessageReachesTheDeadLetterQueue(t *testing.T) {
	ctx := context.Background()
	url := startBroker(t)
	...
	// Consume without auto-ack and reject without requeue, the way a consumer
	// signals "this message is not processable".
	conn, err := amqp.Dial(url)
	if err != nil {
		t.Fatalf("dial: %v", err)
	}
	...
	deliveries, err := ch.Consume("agent.events", "rejecting-consumer", false, false, false, false, nil)
	if err != nil {
	...
	dlq := consume(t, url, "agent.events.dlq", 10*time.Second)
	if dlq.MessageId != ev.EventID {
		t.Errorf("dead-lettered message_id = %q, want %q", dlq.MessageId, ev.EventID)
	}
}
