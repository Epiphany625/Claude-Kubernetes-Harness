...
# debugging both involve kubectl. A `kubectl delete` or a `kubectl patch` typed
# against whatever context happens to be current is the accident worth
# preventing -- the local clusters (minikube, kind, Docker Desktop) are
# disposable, anything else is not.
#
# `kubectl patch` matters here specifically: ops/alerting/alertmanager-patch.yaml
# edits a Helm-managed Alertmanager, and running it against a real monitoring
# stack would silently redirect that cluster's alerting.
#
...
# A server-side dry run changes nothing, and it is how the manifests here are
# validated. Let it through.
if printf '%s' "${COMMAND}" | grep -qE -- '--dry-run(=(server|client))?'; then
  exit 0
fi
